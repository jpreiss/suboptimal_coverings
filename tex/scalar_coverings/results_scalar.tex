...
For the upper bound, whenever possible we use tools that also apply to the matrix case.
It is possible to show the upper bound more directly using the closed-form results
available in the scalar case,
but we retain the matrix-based tools so the proof technique may be more easily extended in future work.
...
\label{lem:petersen-existence}
	For the \DDFproblem\ $(A, U, V, \theta)$, if $B \in \cB$ and $\alpha > 1$,
...
\label{thm:covering-scalar}
	For the scalar \DDFproblem\ defined by
		...
		where $a > 0$,
		and 
		$\cB = \left[\textstyle \recip \breadth, 1 \right]$,
	if $\alpha \geq \frac{2a+1}{2a}$,
	then $\coveringnum{\alpha}{\cB} = O(\log \breadth)$.
\end{theorem}
\begin{proof}
	We construct a cover from the upper end of $\cB$.
	...
	by the intervals $\cB_n = [\beta^{n+1}, \beta^{n}]$
	for $n \in \{0, \dots, N-1\}$,
	and each $\cB_n$ has a controller $k_n$ such that 
	\[
		...
		\right).
	\]
	Then the existence of the desired $\cB_N, k_N$
	...
	Hence, $\cB_n \subseteq \suboptneighb{\alpha}{k_n}$.
	We cover the full $\cB$ when
	$\beta^{N} \leq \recip \theta$,
	which is satisfied by $N \geq - \log \theta / \log \beta$.
\end{proof} 
Note that it may be possible to compute or bound $\beta$ in the scalar case
as a function of $a$ and $\alpha$,
but the analogous result will likely be much more complicated in the matrix case.
...
\label{lem:scalar-neighborhood-optimistic}
	For a scalar \DDFproblem\ with $a \geq 1,\ \cB = [\recip \breadth, 1]$,
	for any $k < 0$,
...
\end{restatable}
\begin{proof}
The result follows from quasiconvexity of
...
\begin{theorem}
\label{thm:scalar-lb}
	For a scalar \DDFproblem\ with
	$a = 1,\ \cB = [\recip \breadth, 1]$,
	if $\alpha \geq \frac 3 2$, %
	then
	$\coveringnum{\alpha}{\cB} = \Omega(\log \breadth)$.
\end{theorem}
\begin{proof}
	Suppose $\coveringset = k_1, \dots, k_N$ is a minimal $\alpha$-suboptimal cover of $\cB$
	in decreasing order.
	From the closed-form solution
	\(
		k^\star_{a,b} = -(a + \sqrt{a^2 + b^2})/b
	\),
	we observe that $k^\star_b < -1$ for all $b \in \cB$.
	This, along with the quasiconvexity of $J_b(k)$ in $k$,
	implies that there exists a minimal cover $\coveringset$ for which all $k_i < -1$.
	For such a cover,
